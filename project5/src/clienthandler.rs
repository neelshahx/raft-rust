use crate::shared::{SenderType, APP_SERVERS};
...
fn handle_client(mut stream: TcpStream, tx: Sender<(SenderType, String)>) {
    loop {
        ...
        tx.send((SenderType::CLIENT, client_cmd.to_string())).unwrap();
    }
}
...
impl ClientHandler {
    ...
    pub(crate) fn listen(&self, tx: Sender<(SenderType, String)>) {
